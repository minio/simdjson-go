...
func testStage2VerifyTape(t *testing.T, filename string) {
	...
	if SLOWGOLANGFLOATPARSING {
		// Tapes to test against were generated by the C simdjson version
		// and contain minor deviations for floats, so when we are using
		// the precise Golang float parsing, ignore these tests
		t.Skip()
	}
